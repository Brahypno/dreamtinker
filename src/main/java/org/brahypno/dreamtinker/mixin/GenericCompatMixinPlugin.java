...
public class GenericCompatMixinPlugin implements IMixinConfigPlugin {
    ...
    private static boolean hasClassResource(String className) {
        ...
        ClassLoader fallback = GenericCompatMixinPlugin.class.getClassLoader();
        return fallback != null && fallback.getResource(path) != null;
    }

    private static String extractCompatModId(String mixinClassName) {
        int idx = mixinClassName.indexOf(COMPAT_SEGMENT);
        if (idx < 0)
            return null;

        int start = idx + COMPAT_SEGMENT.length();
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        String modid = extractCompatModId(mixinClassName);
        if (modid == null)
            return true;
