package org.brahypno.dreamtinker.tools.modifiers.traits.Compat.enigmaticLegacy.material.evil;
...
public class EvilAttack extends Modifier implements ModifyDamageModifierHook, MeleeDamageModifierHook, ConditionalStatModifierHook {
    ...
    public float modifyDamageTaken(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount, boolean isDirectDamage) {
        int cursed = context.getEntity() instanceof Player player ? EnigmaticLegacyCompat.getCurseAmount(player) + 1 : 1;
    ...
    public float getMeleeDamage(@NotNull IToolStackView tool, @NotNull ModifierEntry modifier, @NotNull ToolAttackContext context, float baseDamage, float damage) {
        int cursed = context.getAttacker() instanceof Player player ? EnigmaticLegacyCompat.getCurseAmount(player) + 1 : 1;
    ...
    public float modifyStat(IToolStackView tool, ModifierEntry modifier, LivingEntity living, FloatToolStat stat, float baseValue, float multiplier) {
        if (ToolStats.DRAW_SPEED == stat || ToolStats.VELOCITY == stat){
            int cursed = living instanceof Player player ? EnigmaticLegacyCompat.getCurseAmount(player) + 1 : 1;
            baseValue *= cursed;
