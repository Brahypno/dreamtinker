...
public class CursedRingBound extends NoLevelsModifier implements EquipmentChangeModifierHook, GeneralInteractionModifierHook, TooltipModifierHook {
    ...
    private boolean check(IToolStackView tool, ServerPlayer player) {
        ...
        return player.getAbilities().instabuild ||
               (EnigmaticLegacyCompat.isTheCursedOne(player) &&
                (!worthy_check || EnigmaticLegacyCompat.isTheWorthyOne(player)));
    }
    ...
    public void addTooltip(IToolStackView tool, @NotNull ModifierEntry modifier, @Nullable Player player, List<Component> tooltip, TooltipKey tooltipKey, TooltipFlag tooltipFlag) {
        if (0 < tool.getPersistentData().getInt(TAG_DEEP_CURSE) && tooltipKey.isShiftOrUnknown()){
            EnigmaticLegacyCompat.indicateWorthyOnesOnly(tooltip, player);
        }else {
            EnigmaticLegacyCompat.indicateCursedOnesOnly(tooltip, player);
        }
    }
