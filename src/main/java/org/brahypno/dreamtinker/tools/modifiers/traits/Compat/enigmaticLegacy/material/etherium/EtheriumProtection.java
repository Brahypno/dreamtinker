package org.brahypno.dreamtinker.tools.modifiers.traits.Compat.enigmaticLegacy.material.etherium;
...
public class EtheriumProtection extends Modifier implements DamageBlockModifierHook, ModifyDamageModifierHook {
    ...
    private boolean has_shield(Player player) {
        return ETModifierCheck.ModifierALLBody(player, this.getId()) &&
               player.getHealth() / player.getMaxHealth() <= EnigmaticLegacyCompat.etheriumShieldThresholdMultiplier(player);
    }
    ...
    public boolean isDamageBlocked(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount) {
        if (context.getEntity() instanceof Player player && !player.level().isClientSide && EquipmentSlot.CHEST == slotType)
            if (has_shield(player) && (source.getDirectEntity() instanceof AbstractHurtingProjectile || source.getDirectEntity() instanceof AbstractArrow)){
                player.level().playSound(null, player.blockPosition(), EnigmaticLegacyCompat.etheriumShieldTriggerSound(), SoundSource.PLAYERS, 1.0F,
    ...
    public float modifyDamageTaken(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount, boolean isDirectDamage) {
        if (context.getEntity() instanceof Player player && !player.level().isClientSide && EquipmentSlot.CHEST == slotType)
            if (has_shield(player)){
                ...
                amount *= (float) EnigmaticLegacyCompat.etheriumShieldReductionModifierInverted();
            }
