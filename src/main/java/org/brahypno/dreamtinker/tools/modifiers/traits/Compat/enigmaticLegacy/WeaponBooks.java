...
import static org.brahypno.dreamtinker.tools.modifiers.traits.Compat.enigmaticLegacy.EldritchPan.getBloodlust;
...
public class WeaponBooks extends Modifier implements MeleeDamageModifierHook, MeleeHitModifierHook, ValidateModifierHook {
    ...
    public float beforeMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damage, float baseKnockback, float knockback) {
        ...
        if (context.getAttacker() instanceof Player player && !player.level().isClientSide){
            if (0 < tool.getModifierLevel(this.getId()) && EnigmaticLegacyCompat.isTheCursedOne(player)){
                context.getTarget().setSecondsOnFire(20);
            }
            if (1 == tool.getModifierLevel(this.getId()) && EnigmaticLegacyCompat.isTheCursedOne(player)){
                knockbackPower += EnigmaticLegacyCompat.twistKnockbackBonusModifier();
            }else if (2 <= tool.getModifierLevel(this.getId()) && EnigmaticLegacyCompat.isTheWorthyOne(player)){
                if (3 <= tool.getModifierLevel(this.getId()) && EnigmaticLegacyCompat.isTheWorthyOne(player))
                    knockbackPower += EnigmaticLegacyCompat.twistKnockbackBonusModifier();
                knockbackPower += EnigmaticLegacyCompat.infinitumKnockbackBonusModifier();
            }
        }
    ...
    public float getMeleeDamage(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float baseDamage, float damage) {
        ...
        if (context.getAttacker() instanceof Player player){
            if (EnigmaticLegacyCompat.isTheCursedOne(player) || player instanceof ServerPlayer sp && sp.getAbilities().instabuild){
                LivingEntity target = ETHelper.getLivingTarget(context.getTarget());
                if (EnigmaticLegacyCompat.isBossOrPlayer(target))
                    if (1 == tool.getModifierLevel(this.getId())){
                        bonus += damage * EnigmaticLegacyCompat.twistBossDamageBonusModifier();
                    }else if (2 <= tool.getModifierLevel(this.getId())){
                        if (3 <= tool.getModifierLevel(this.getId()))
                            bonus += damage * EnigmaticLegacyCompat.twistBossDamageBonusModifier();
                        bonus += damage * EnigmaticLegacyCompat.infinitumBossDamageBonusModifier();
                    }
            }else {
                damage = 0;
    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        if (2 <= tool.getModifierLevel(this.getId()))
            if (context.getAttacker() instanceof Player player && !player.level().isClientSide
                && (EnigmaticLegacyCompat.isTheWorthyOne(player) || player instanceof ServerPlayer sp && sp.getAbilities().instabuild)){
                ...
                if (null != getBloodlust() && player.hasEffect(getBloodlust())){
                    ...
                    lifesteal += (float) (damageDealt * (EnigmaticLegacyCompat.bloodlustLifestealBoost() * amplifier));
                }
    ...
    private void onLivingDeath(LivingDeathEvent event) {
        if (event.getEntity() instanceof Player player && !event.getEntity().level().isClientSide){
            if (2 <= ETModifierCheck.getMainhandModifierLevel(player, this.getId()) &&
                Math.random() <= EnigmaticLegacyCompat.infinitumUndeadProbabilityMultiplier()){
