...
public class moonlightConversion {
    ...
    public static void onEntityJoinWorld(EntityJoinLevelEvent event) {
        ...
        if (item.getItem().getItem() != Items.BLUE_ICE) return;
        // 跟踪蓝冰
    ...
    public static void onWorldTick(TickEvent.LevelTickEvent event) {
        ...
        while (iterator.hasNext()) {
            ...
            if (item.isInWater()) {
                if(!isAllowedPhase) {
                    level.playSound(null, item.blockPosition(), SoundEvents.NOTE_BLOCK_BASS, SoundSource.BLOCKS, 0.5F, 0.8F);
    ...
    private static ItemStack getRandomMoonlightIceHeadPart() {
        ...
        List<ToolPartItem> headParts = ForgeRegistries.ITEMS.getValues().stream()
                .filter(item -> item instanceof ToolPartItem part
                        && part.getStatType() == HeadMaterialStats.ID)
                .map(item -> (ToolPartItem) item)
                .toList();


        if (headParts.isEmpty()) return ItemStack.EMPTY;
