...
public class DreamtinkerRecipeProvider extends RecipeProvider implements IMaterialRecipeHelper, IToolRecipeHelper, IConditionBuilder, IRecipeHelper {
    ...
    private void addPartRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        ArrayList<CastItemObject> casts = new ArrayList<>(
                Arrays.asList(TinkerSmeltery.helmetPlatingCast, TinkerSmeltery.bootsPlatingCast, TinkerSmeltery.chestplatePlatingCast,
                              TinkerSmeltery.leggingsPlatingCast));
        ArmorItem.Type[] armor_items = {ArmorItem.Type.HELMET, ArmorItem.Type.BOOTS, ArmorItem.Type.CHESTPLATE, ArmorItem.Type.LEGGINGS};
        TagKey<Item>[] ai =
                new TagKey[]{tconItemTag("helmet_plating"), tconItemTag("boots_plating"), tconItemTag("chestplate_plating"), tconItemTag("leggings_plating")};
        int[] costs = {3, 2, 6, 5};
        CompoundTag nbt = new CompoundTag();
        nbt.putString("Material", "dreamtinker:star_regulus");
        for (int i = 0; i < casts.size(); i++) {
            ItemStack stack = new ItemStack(casts.get(i));
