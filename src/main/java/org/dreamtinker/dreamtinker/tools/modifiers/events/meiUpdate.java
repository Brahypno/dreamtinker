...
public class meiUpdate {
    ...
    public static void onPlayerLevelChange(PlayerXpEvent.LevelChange event) {
        ...
        List<ItemStack> hits = findStacksWithModifier(player, DreamtinkerModifiers.mei.getId());

        if (!hits.isEmpty()){
            for (ItemStack stack : hits) {
                ToolStack ts = ToolStack.from(stack);
                ts.addModifier(DreamtinkerModifiers.mei.getId(), event.getLevels());//I know this may cause thread overwritten,but who cares
